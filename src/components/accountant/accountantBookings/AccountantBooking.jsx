...
export default class AccountantBooking extends Component {
  state = {
    hotels: [],
    searchfield: "",
    loading: true,
  ...
  async componentDidMount() {
    try {
      const response = await axios.get(
        `https://calm-anchorage-14244.herokuapp.com/admin/acctgetAllHotels`
      );
              ...
              {newHotels.map((item, index) => {
                return (
                  <tr key={`${item._id}-${index}`}>
                    ...
                    <td>
                      <Link to={`/account/booking/${item._id}`}>
                        {" "}
                        View Invoice
                      </Link>
                    </td>
